#!/bin/bash
#SBATCH --job-name=lighteval_qwen25_7b
#SBATCH --mem=80G
...
log_base_dir="/path/to/cva-merge/evaluation/logs/source/vanilla"
        ...
        if [[ $task == "custom|"* ]]; then
            ...
            if [[ $task == "lighteval|belebele_eng_Latn_mcf|3|0" ]]; then
                ...
                lighteval accelerate \
                    ...
                    --output_dir="${log_base_dir}/${model_abbrev}/${task_name}" \
                    --cache_dir="${cache_dir}"
            elif [[ $task == "lighteval|gsm8k|0|0"  ]]; then
